...
export class AudioManager {
  ...
  source.onended = () => {
    ...
    // Resolve waiters only when every scheduled source has finished.
  ...
  // Nothing is currently playing/scheduled.
  if (this.sources.size === 0) {
    return;
  }
...
interrupt() {
  ...
  // Do not leave anyone waiting forever if playback was interrupted.
  const waiters = this.playbackWaiters.splice(0);
  for (const resolve of waiters) {
    resolve();
  }
